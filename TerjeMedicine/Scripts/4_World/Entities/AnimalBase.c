...
modded class AnimalBase
{
	...
	void AnimalBase()
	{
		RegisterNetSyncVariableInt("m_terjeRadiationSynch", 0, TerjeMedicineConstants.RADIATION_ANIMALS_ACCUMULATOR_SYNCH_MAX);
	}
	...
	override bool AddTerjeRadiation(float rAmount)
	{
		...
		{
			m_terjeRadiationServer = Math.Clamp(m_terjeRadiationServer + rAmount, 0, TerjeMedicineConstants.RADIATION_ANIMALS_ACCUMULATOR_SERVER_MAX);
			
			int newRadiationSynchValue = (int)Math.Round(m_terjeRadiationServer / TerjeMedicineConstants.RADIATION_ANIMALS_ACCUMULATOR_SYNCH_DIVIDER);
	...
	override float GetTerjeRadiation()
	{
		...
		{
			return m_terjeRadiationSynch * TerjeMedicineConstants.RADIATION_ANIMALS_ACCUMULATOR_SYNCH_DIVIDER;
		}
	};
	...
	override void CommandHandler(float dt, int currentCommandID, bool currentCommandFinished)
	{
		...
		if (GetGame().IsDedicatedServer())
		{
			...
			if (m_terjeRadiationServer > TerjeMedicineConstants.RADIATION_ANIMALS_DAMAGE_THRESHOLD && GetTerjeSettingBool(TerjeSettingsCollection.MEDICINE_RADIATION_DAMAGE_ANIMALS))
			{
				DecreaseHealth("", "", TerjeMedicineConstants.RADIATION_ANIMALS_DAMAGE_PER_SEC * dt);
			}
		}
	}
