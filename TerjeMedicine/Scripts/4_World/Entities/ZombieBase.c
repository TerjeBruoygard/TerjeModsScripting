...
modded class ZombieBase
{
	...
	void ZombieBase()
	{
		RegisterNetSyncVariableInt("m_terjeRadiationSynch", 0, TerjeMedicineConstants.RADIATION_ZOMBIE_ACCUMULATOR_SYNCH_MAX);
	}
	...
	override bool AddTerjeRadiation(float rAmount)
	{
		...
		{
			m_terjeRadiationServer = Math.Clamp(m_terjeRadiationServer + rAmount, 0, TerjeMedicineConstants.RADIATION_ZOMBIE_ACCUMULATOR_SERVER_MAX);
			
			int newRadiationSynchValue = (int)Math.Round(m_terjeRadiationServer / TerjeMedicineConstants.RADIATION_ZOMBIE_ACCUMULATOR_SYNCH_DIVIDER);
	...
	override float GetTerjeRadiation()
	{
		...
		{
			return m_terjeRadiationSynch * TerjeMedicineConstants.RADIATION_ZOMBIE_ACCUMULATOR_SYNCH_DIVIDER;
		}
	}
	...
	override void CommandHandler(float pDt, int pCurrentCommandID, bool pCurrentCommandFinished)
	{
		...
		if (GetGame().IsDedicatedServer())
		{
			...
			if (m_terjeRadiationServer > TerjeMedicineConstants.RADIATION_ZOMBIE_DAMAGE_THRESHOLD && GetTerjeSettingBool(TerjeSettingsCollection.MEDICINE_RADIATION_DAMAGE_ZOMBIES))
			{
				DecreaseHealth("", "", TerjeMedicineConstants.RADIATION_ZOMBIE_DAMAGE_PER_SEC * pDt);
			}
		}
	}
