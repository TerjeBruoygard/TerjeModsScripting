...
За пример возмем медведя:
1. Создаем config.cpp в корне своего мода **TerjeModding** на диске P. 
2. Так как мы собираемся переопределить существующее животное, то нам нужно найти название мода (класса CfgPatches) в котором он находится, чтобы прописать его в **requiredAddons** (так же нужно сделать и с любым другим животныт, зомби, мутантом из стороннего мода).
	...
	class YourCustomAnimal: AnimalBase ///Ваш класс Животного или Зомби:  class YourCustomZmb: ZombieBase 
	{
		class TerjeCustomInjuries
		{
			...
			// Перезаписывать раны по умолчанию ранами из конфигурации или дополнять их.
            		...
            		// Если 0 - то к ранениям по дефолту (прописаные на тому или иному типу животных и зомби - колотые раны, ушибы и т.д.) будут дополнительно добавляться ранения из этого конфига. 
...
Разберем пример **config.cpp** на модифицирование модового ножа - **TerjeModKnife_1**, **TerjeModKnife_2**, ванильного - **HuntingKnife**.
```cs
class CfgPatches
{
	class TerjeModding
...
class CfgVehicles
{
	...
	class HuntingKnife: Inventory_Base // Ванильный Костяной нож
	{
    	...
    	class TerjeModKnife_1: Inventory_Base // Модовый нож 5 секунд
	{
    	...
    	class TerjeModKnife_2: Inventory_Base // Модовый нож с быстрой резкой
	{
