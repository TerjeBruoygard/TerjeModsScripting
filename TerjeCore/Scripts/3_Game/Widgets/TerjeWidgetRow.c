// <copyright file="TerjeWidgetRow.c" author="Terje Bruoygard">
...
class TerjeWidgetRow : TerjeWidgetBase
{
	...
	{
		PushPostCommand(new TerjeWidgetCommand_RowRecalc());
	}
	...
	void RecalculateLayoutImmediately()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (nativeChild != null)
			{
				...
				float width = ownerWidth * (weight / m_totalWeight);
				nativeChild.SetScreenPos(ownerLeft + offset, ownerTop);
				nativeChild.SetScreenSize(width, ownerHeight);
				offset = offset + width;
			}
		}
	}
	...
	override void OnCommand(TerjeWidgetCommand command, float timeslice)
	{
		...
		if (command.IsInherited(TerjeWidgetCommand_RowRecalc) && m_totalWeight > 0)
		{
...
class TerjeWidgetCommand_RowRecalc : TerjeWidgetCommand
{
}