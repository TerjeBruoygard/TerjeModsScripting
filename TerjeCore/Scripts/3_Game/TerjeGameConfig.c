...
class TerjeGameConfig
{
	...
	protected void BuildCache()
	{
		...
		if (m_config != null)
		{
			...
			{
				BuildCacheObject(m_config.GetChild(i), string.Empty);
			}
		}
		
		if (m_cache.Count() > 0)
		{
			TerjeLog_Info("Overrided " + m_cache.Count() + " values from config.cpp");
		}
	}
	
	protected void BuildCacheObject(TerjeXmlObject obj, string path)
	{
		if ((obj != null) && (obj.IsObjectNode()) && (obj.GetName() != string.Empty))
		{
			...
			else if (obj.HasChildren())
			{
				...
				{
					BuildCacheObject(obj.GetChild(i), path);
				}
			}
